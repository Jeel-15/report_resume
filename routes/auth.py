...
def google_login():
    ...
    if not google_client_id:
        return jsonify({'message': 'Google Sign-In is not configured on server'}), 503

    if google_id_token is None or google_requests is None:
        return jsonify({'message': 'Google Sign-In dependency is missing on server'}), 503
    ...
    if not email or not email_verified or not _validate_email(email):
        return jsonify({'message': 'Google account email is invalid or not verified'}), 401
    ...
    if not user:
        ...
        # Social accounts do not have a local password by default.
