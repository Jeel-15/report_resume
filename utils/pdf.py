...
def _build_overlay_page(page_width, page_height, header_left, header_right, footer_student_name, page_number):
    writer = PdfWriter()
    ...
    resources = DictionaryObject({
        NameObject('/Font'): DictionaryObject({
            NameObject('/F1'): font_regular,
            NameObject('/F2'): font_bold,
        })
    })
    page[NameObject('/Resources')] = resources

    # Centered layout: keep 1in left/right/bottom margins in points.
    left_margin = 72
    right_margin = 72
    bottom_margin = 72

    footer_y = bottom_margin - 20
    footer_rule_y = bottom_margin - 10

    overlay_lines = [
        f'{left_margin} {footer_rule_y} m {page_width - right_margin} {footer_rule_y} l S',
        'BT /F1 7 Tf',
        f'1 0 0 1 {left_margin} {footer_y} Tm ({_escape_pdf_text(footer_student_name)}) Tj ET',
        'BT /F1 7 Tf',
        f'1 0 0 1 {page_width - right_margin - 60} {footer_y} Tm (Page no. {page_number}) Tj ET',
    ]

    stream = DecodedStreamObject()
    stream.set_data(('\n'.join(overlay_lines)).encode('latin-1'))
...
def generate_pdf_from_html(html_content, base_url=None, student_name='Student'):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Extract first header/footer labels for running header/footer overlays.
            ...
            rendered_html = re.sub(
                r'(?is)<div\s+class="page-header">.*?</div>',
                '',
                rendered_html,
            )
            rendered_html = re.sub(
                r'(?is)<div\s+class="page-footer">.*?</div>',
                '',
                rendered_html,
            )
            ...
            if len(reader.pages) > 1:
                ...
                pdf_bytes = merged_output.getvalue()
